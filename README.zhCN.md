...
- **API 端点**：`GET /_auth`（认证检查）、`GET /_login`（登录页）、`POST /_login`（登录）、`POST /oauth/device/authorization`（设备授权）、`GET/POST /device`（浏览器确认）、`POST /oauth/token`（设备码/刷新令牌兑换）、`POST /_send_verify_code`（发送 OTP）、`GET /_logout`（登出）、`GET /_session_exchange`（跨域）、`GET /totp/enroll`、`POST /totp/enroll/confirm`、`GET /totp/revoke`、`POST /totp/revoke`（Herald TOTP 启用时）、`GET /health`（健康检查）、`GET /metrics`（Prometheus）
